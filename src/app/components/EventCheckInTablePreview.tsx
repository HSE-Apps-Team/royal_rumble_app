...
// Demo-only version of EventCheckInTable for the admin mentor-preview pages.
// Those pages show hardcoded example data with no real DB rows behind them,
// so this simulates a successful check-in locally instead of calling the
// real checkMentorAttendanceCode server action.
export default function EventCheckInTablePreview({
  events,
}: {
  events: EventRow[];
}) {
    ...
    <>
      ...
      {activeEvent && (
        <ModalShell
          ...
          footer={
            <>
              <button style={makeBtn("var(--secondarySilver)")} onClick={closeModal}>
                Cancel
              </button>
              <button style={makeBtn("var(--primaryBlue)")} onClick={submitCode}>
                Submit
              </button>
            </>
          }
